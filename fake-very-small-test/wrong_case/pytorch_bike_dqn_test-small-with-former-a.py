...
class Env(object):
    ...
    def step(self, action):
        ...
        if move + self.obs[-self.region_num-2+region] and move + self.obs[-self.region_num-2+region] >= 0 and move <= self.obs[-1]:
        ...
        reward = np.sum(self.obs[-self.region_num-2:-2][self.obs [-self.region_num-2:-2]< 0])+10
...
class Dqn():
    ...
    def learn(self):
        ...
        if self.learn_counter % 50 == 0:
            ...
            print(np.mean(action_val.cpu().detach().numpy()), file=open(f"result_history/output_action_value_{ts}.txt", "a"))
        ...
        print(loss.item(), file=open(f"result_history/output_loss_{ts}.txt", "a"))
        ...
        for i in range(1):
            ...
            while True:
                ...
                print(
                    ...
                    file=open(f"result_action/output_action_{ts}.txt", "a"))
                # if render:
                #     env.render()
                if done:
                    break
...
def main():
    ...
    net = Dqn(NUM_STATES, NUM_ACTIONS, env.region_num, env.move_amount_limit)
        ...
        while True:
            ...
            if net.memory_counter >= 5*BATCH_SIZE:
                ...
                if done:
                    ...
                    print(f"{round(reward_sum, 3)}", file=open(f"result_history/output_result_{ts}.txt", "a"))
