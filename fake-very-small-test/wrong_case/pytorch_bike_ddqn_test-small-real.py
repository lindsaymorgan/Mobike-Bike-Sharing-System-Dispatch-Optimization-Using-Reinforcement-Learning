...
class Env(object):
    def __init__(self, region_num, move_amount_limit, eps_num):
        ...
        self.t = 0
        self.obs_init = np.array([500, 500, 500, 500, 0, 0,0,0,500,500, 500, 500, 0, 0])  # 各方格单车量+货车位置+货车上的单车量
        ...
        for i in range(1):
            ...
            while True:
                ...
                print(
                    ...
                    f"move:{(action % (2 * self.move_amount_limit + 1) - self.move_amount_limit)*10} reward:{reward} "
                    f"reward_sum:{episode_reward}",
                    file=open(f"result_action/ddqn_real_output_action_{ts}.txt", "a"))
                # if render:
                #     env.render()
                if done:
                    break
        ...
        while True:
            ...
            if net.memory_counter >= MEMORY_CAPACITY:
                ...
                if done:
                    ...
                    print(f"{round(reward_sum, 3)}", file=open(f"result_history/ddqn_real_output_result_{ts}.txt", "a"))
