...
class Env(object):
    ...
    def check_limit(self,arg):  #对一个state(不含t,14位),action,t(当前)是否合法 Todo:check
        ...
        move = (action % (2 * self.move_amount_limit + 1) - self.move_amount_limit)*20
...
class Net(nn.Module):
    def __init__(self, NUM_STATES,car_num):
        ...
        self.fc1 = nn.Linear(OTHER_SIZE + EMB_SIZE * (2*car_num+2), 1024).cuda()
        # self.fc1.weight.data.normal_(0, 0.1)
        self.fc2 = nn.Linear(1024, 256).cuda()
        # self.fc2.weight.data.normal_(0, 0.1)
        self.fc3 = nn.Linear(256, 64).cuda()
        # self.fc3.weight.data.normal_(0, 0.1)
        self.fc4 = nn.Linear(64, 1).cuda()
        ...
        for i in range(1):
            ...
            while True:
                ...
                print(
                    ...
                    f"move:{20*(action % (2 * self.move_amount_limit + 1) - self.move_amount_limit)} reward:{reward} "
...
def main():
    ...
    EPSILON = 0.99
    EPS_DECAY = 0.999
    env = Env(region_num=58, move_amount_limit=25, eps_num=eps_num,car_num=car_num)
    ...
    for episode in tqdm(range(EPISODES)):
        ...
        while True:
            ...
            move = (action % (2 * env.move_amount_limit + 1) - env.move_amount_limit)*20
            ...
            if done:
                ...
                print(f"{round(reward_sum, 3)}", file=open(f"result_history/subgraph32_real_output_result_move_amount_limit50_{ts}.txt", "a"))
                break
