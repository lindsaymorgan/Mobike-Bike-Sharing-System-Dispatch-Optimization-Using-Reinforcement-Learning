...
class Env(object):
    ...
    def check_limit(self,arg):  #对一个state(不含t,14位),action,t(当前)是否合法 Todo:check
        ...
        move = (action % (2 * self.move_amount_limit + 1) - self.move_amount_limit)*car_batch
...
def main():
    ...
    EPSILON = 0.99
    EPS_DECAY = 0.999
    env = Env(region_num=58, move_amount_limit=10, eps_num=eps_num,car_num=car_num)
    ...
    for episode in tqdm(range(EPISODES)):
        ...
        while True:
            ...
            move = (action % (2 * env.move_amount_limit + 1) - env.move_amount_limit)*car_batch
