...
class Env(object):
    def __init__(self, region_num, move_amount_limit, episode_num):
        ...
        self.t_index = {i: str(i) for i in range(24)}
        self.out_nums = np.array([self.start_region[str(i)].agg(np.sum) for i in range(24)])
        self.in_nums = np.array([self.end_region[str(i)].agg(np.sum) for i in range(24)])
    ...
    def step(self, action):
        ...
        # 更新单车分布状态
        # 处理上时段骑入
        self.obs[:self.region_num] += self.in_nums[self.t - 1, :self.region_num]
        ...
        self.obs[:self.region_num] -= out_num[self.t, :self.region_num]
        reward = np.sum(self.obs[self.obs < 0])
    ...
    def choose_action(self, state):
        ...
        if np.random.randn() <= EPSILON:
            ...
            for action in feasible_action:
                ...
                x = torch.FloatTensor(np.concatenate([a_t, bike_num_t, move])).cuda()  # 区块单车量，时刻t，搬运量
    ...
    def learn(self):
        ...
        for i in batch_next_state:
            ...
            for action in feasible_action:
                ...
                tmp_x.append(np.concatenate([a_t, bike_num_t, move]))
